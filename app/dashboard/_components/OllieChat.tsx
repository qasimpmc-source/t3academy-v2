...
const DEFAULT_WELCOME = "Hey! 👋 I'm Ollie. What are you working on today?";

export default function OllieChat({ context, welcomeMessage, quickPrompts }: Props) {
  ...
  // Reset chat when context changes (e.g. new quiz question)
  useEffect(() => {
    ...
    <div className="flex flex-col h-full" style={{ background: "var(--color-card)" }}>
      ...
      >
        <div
          ...
          style={{ background: "var(--color-amber-bg)", border: "1px solid rgba(212,134,10,0.2)" }}
        >
          🦉
        </div>
        <div>
          <div className="text-sm font-semibold" style={{ color: "var(--color-text)" }}>Ollie</div>
          <div className="flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full" style={{ background: "#22c55e" }} />
            <span className="text-xs" style={{ color: "var(--color-text3)" }}>Online · 11+ tutor</span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <button
          ...
          style={{ background: "var(--color-amber)", color: "#fff", cursor: "pointer", border: "none" }}
        >
